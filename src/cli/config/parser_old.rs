...
impl<'a> OldParser<'a> {
    ...
    fn parse_entry(&mut self, entry_rules: Vec<RuleGroup>, filter: RuleFilter, verbatim: String, file_path: PathBuf, rule_file: &str) -> io::Result<Entry> {
    ...
    fn get_entry(&mut self) -> io::Result<Option<Entry>> {
        ...
        if file_path.is_file(){
            ...
            match filter {
                Some(rf) => Ok(Some(self.parse_entry(entry_rules, rf, verbatim, file_path, rule_file)?)),
